...
public class MusikBotMover extends Event {
    private final int botstorage = TeamSpeakBot.getConfig().getInt("musikbotrent.botstorage");
    private final int rentgroup = TeamSpeakBot.getConfig().getInt("musikbotrent.rentgroup");
    private final List<String> groups = (List<String>) TeamSpeakBot.getConfig().getList("musikbotrent.channelgroups");
    ...
    public void ClientMove(ClientMovedEvent event) {
        ...
        if (groups.contains(String.valueOf(TeamSpeakBot.api.getClientInfo(iid).getChannelGroupId()))
            && SupportBot.rentingMusic.contains(iid)
        ) {
