...
public class ToManyIPs extends Event {
    HashMap<String, Integer> ips = new HashMap<>();
    HashMap<Integer, ClientInfo> clientinfo = new HashMap<>();
    ...
    public void JoinEvent(ClientJoinEvent event) {
        ...
        if (ips.containsKey(ip)) {
            ...
            if (ips.get(ip) > maxclients) {
                ...
                Logger.write("To many users on the same IP. Banned: " + ci.getNickname(), -1);
            }
        } else {
            ips.put(ip, 1);
        }
